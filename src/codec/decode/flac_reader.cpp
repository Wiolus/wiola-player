...
FlacReader::FlacReader(pcm::StreamSpec spec, pcm::Frames num_frames,
...
std::unique_ptr<FlacReader> FlacReader::open(const std::filesystem::path& path)
{
    ...
    const pcm::StreamSpec spec{.sample_rate = units::Frequency{flac->sampleRate},
    ...
    return std::unique_ptr<FlacReader>{
        new FlacReader{spec, pcm::Frames{num_frames}, std::move(handle)}
    };
}

std::size_t FlacReader::decode(std::span<float> output, pcm::Frames num_frames)
{
    return static_cast<std::size_t>(drflac_read_pcm_frames_f32(handle_->flac, num_frames.count(),
        output.data()));
}

bool FlacReader::seek_frame(pcm::Frames frame_index)
{
